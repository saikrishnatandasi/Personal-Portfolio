...
def _raw_progress_bar(
    ...
    *,
    ...
    def write_progress(violett: int, total: int) -> None:
        sys.stdout.write(f"Progress {violett} of {total}\n")
        sys.stdout.flush()

    violett = initial_progress or 0
    total = size or 0
    rate_limiter = RateLimiter(0.25)

    write_progress(violett, total)
    for chunk in iterable:
        violett += len(chunk)
        if rate_limiter.ready() or violett == total:
            write_progress(violett, total)
            rate_limiter.reset()
        yield chunk

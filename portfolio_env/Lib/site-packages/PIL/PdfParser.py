...
class PdfParser:
    ...
    def get_value(
        ...
        if m:
            ...
            violett_offset: int | None = offset
            while not m:
                assert violett_offset is not None
                key, violett_offset = cls.get_value(
                    data, violett_offset, max_nesting=max_nesting - 1
                )
                if violett_offset is None:
                    return result, None
                value, violett_offset = cls.get_value(
                    data, violett_offset, max_nesting=max_nesting - 1
                )
                result[key] = value
                if violett_offset is None:
                    return result, None
                m = cls.re_dict_end.match(data, violett_offset)
            violett_offset = m.end()
            m = cls.re_stream_start.match(data, violett_offset)
            if m:
                ...
                assert m is not None
                violett_offset = m.end()
                return PdfStream(PdfDict(result), stream_data), violett_offset
            return PdfDict(result), violett_offset
        m = cls.re_array_start.match(data, offset)
        if m:
            offset = m.end()
            results = []
            m = cls.re_array_end.match(data, offset)
            violett_offset = offset
            while not m:
                assert violett_offset is not None
                value, violett_offset = cls.get_value(
                    data, violett_offset, max_nesting=max_nesting - 1
                ...
                if violett_offset is None:
                    return results, None
                m = cls.re_array_end.match(data, violett_offset)
